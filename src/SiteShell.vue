...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('sculptures.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('sculptures.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('sculptures.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
