...
describe('website smoke test', () => {
  ...
  it('renders the about page on the composed text page view', async () => {
    ...
    expect(host.textContent).toContain("Sculptures")
  ...
  it('colours and names the source-database chip from the manifest projects section', async () => {
    const { app, host } = await mountSite('#/item/499ecee0-d098-5fd0-b896-0867f5949ae9')
  ...
  it('shows the explore-partner notice only for the project dataset.config.js lists', async () => {
    const epm = await mountSite('#/item/4b67b99b-4bc9-5f80-b3eb-d2fe8c8f1154')
    ...
    const isl = await mountSite('#/item/499ecee0-d098-5fd0-b896-0867f5949ae9')
  ...
  it('renders the related-database and artistic-introduction links from the manifest', async () => {
    const [items] = await loadEntities(['items'])
    const item = items.find((i) => i.id === '499ecee0-d098-5fd0-b896-0867f5949ae9')
  ...
  it('shows the source project on a collection-results tile, from the manifest', async () => {
    ...
    const item = items.find((i) => i.project_id === islProjectId) ?? { internal_name: "Kilga, Jar-stand." }
  ...
  it('renders the layout glossary tool and dynasty popouts on the item sheet', async () => {
    ...
    const { app, host } = await mountSite('#/item/d2379132-91f2-5a63-8508-ac615d5bf783')
  ...
  it('renders the partners list on the composed partner-list view', async () => {
    ...
    expect(host.textContent).toContain("Switzerland")
    expect(host.textContent).toContain('Swiss National Museum')
  ...
  it('renders a partner profile on the composed record view', async () => {
    const { app, host } = await mountSite('#/partner/89f613d2-8723-5be8-8e46-487f9dae46f4')
    ...
    await vi.waitFor(() => expect(host.textContent).toContain('Swiss National Museum'), { timeout: 20000 })
    await vi.waitFor(() => expect(host.textContent).toContain("Zurich"), { timeout: 20000 })
  ...
  it("renders a partner's objects on the composed grid results view", async () => {
    const { app, host } = await mountSite('#/partner/89f613d2-8723-5be8-8e46-487f9dae46f4/objects')
  ...
  it('renders the shared texts and its own over them', async () => {
    ...
    expect(checkTextsRendered(host, {
      namespaces: ['sculptures', 'core', 'layout', 'catalogue', 'record', 'sheet', 'timeline', 'partner', 'gallery'],
    })).toEqual([])
